...
uint64_t on9rstore::get_entry_size(uint32_t payload_len)
{
    const uint64_t base_size =
        sizeof(on9rstore_def::entry_header) +
        static_cast<uint64_t>(payload_len) +
        on9rstore_def::entry_crc_len;
    if (base_size >
        SIZE_MAX - (on9rstore_def::entry_alignment - 1)) {
        return 0;
    }

    return on9rstore_def::align_up(
        static_cast<size_t>(base_size),
        on9rstore_def::entry_alignment);
}
...
uint64_t on9rstore::make_next_entry_id_unsafe()
{
    ...
    newest_entry_id =
        (static_cast<uint64_t>(state.boot_counter) << 40ULL) |
        state.next_entry_sequence;
    state.newest_entry_id = newest_entry_id;
    return newest_entry_id;
}

esp_err_t on9rstore::build_entry_header_unsafe(
    ...
    on9rstore_def::entry_header *header_out)
{
    ...
    header.segment_slot =
        static_cast<uint16_t>(active_segment.slot);
    header.segment_generation = active_segment.generation;
    *header_out = header;
    return ESP_OK;
}

void on9rstore::add_sparse_index_entry_unsafe(
    const on9rstore_def::entry_header &header, uint64_t offset)
{
    if (sparse_index_count >= sparse_index_capacity ||
        offset > UINT32_MAX) {
        return;
    }

    on9rstore_def::sparse_index_entry &index =
        sparse_index[sparse_index_count];
    index = {};
...
void on9rstore::account_appended_entry_unsafe(
    const on9rstore_def::entry_header &header, uint64_t offset,
    uint64_t entry_size)
{
    segment_descriptor &descriptor =
        segments[active_segment.slot];
    if (descriptor.entry_count %
            active_segment.index_stride ==
        0) {
...
esp_err_t on9rstore::prepare_entry_space_unsafe(uint64_t entry_size)
{
    const uint64_t capacity =
        active_segment.data_end -
        active_segment.data_start;
    if (entry_size > capacity) {
        return ESP_ERR_INVALID_SIZE;
    }

    if (entry_size <=
        active_segment.data_end - active_write_offset) {
        return ESP_OK;
    }
...
esp_err_t on9rstore::write_zeroes_unsafe(
    ...
    uint64_t len) const
{
    ...
    while (len > 0) {
        const size_t chunk_len =
            len < sizeof(zeroes) ? static_cast<size_t>(len) :
                                   sizeof(zeroes);
        esp_err_t ret = write_exact_fd(file_fd, file_size, offset,
                                       zeroes, chunk_len);
        if (ret != ESP_OK) {
            return ret;
        }
...
esp_err_t on9rstore::write_entry_trailer_unsafe(
    uint64_t entry_offset, uint32_t payload_len, uint32_t crc,
    uint64_t entry_size)
{
    const uint64_t crc_offset =
        entry_offset + sizeof(on9rstore_def::entry_header) +
        payload_len;
    esp_err_t ret = write_exact_fd(writer_fd, segment_file_size,
                                   crc_offset, &crc, sizeof(crc));
    if (ret != ESP_OK) {
        return ret;
    }
    ...
    const uint64_t written =
        sizeof(on9rstore_def::entry_header) +
        static_cast<uint64_t>(payload_len) + sizeof(crc);
    if (entry_size < written) {
        return ESP_ERR_INVALID_SIZE;
    }

    return write_zeroes_unsafe(writer_fd, segment_file_size,
                               entry_offset + written,
                               entry_size - written);
}

esp_err_t on9rstore::append_buffered_entry_unsafe(
    uint16_t type, const uint8_t *payload, uint32_t payload_len,
    on9rstore_def::entry_header *entry_info_out, bool force_flush)
{
    ...
    const uint32_t checksum =
        calc_crc32(entry, sizeof(header) + payload_len);
    memcpy(entry + sizeof(header) + payload_len,
...
esp_err_t on9rstore::append_direct_entry_unsafe(
    uint16_t type, const uint8_t *payload, uint32_t payload_len,
    on9rstore_def::entry_header *entry_info_out)
{
    ...
    const uint64_t entry_offset = active_write_offset;
    ret = write_exact_fd(writer_fd, segment_file_size, entry_offset,
                         &header, sizeof(header));
    uint32_t crc = calc_crc32_update(
        UINT32_MAX, reinterpret_cast<const uint8_t *>(&header),
        sizeof(header));
    if (ret == ESP_OK && payload_len > 0) {
        ret = write_exact_fd(writer_fd, segment_file_size,
                             entry_offset + sizeof(header),
                             payload, payload_len);
        crc = calc_crc32_update(crc, payload, payload_len);
    }
    if (ret == ESP_OK) {
        ret = write_entry_trailer_unsafe(
            entry_offset, payload_len, ~crc, entry_size);
    }
    if (ret == ESP_OK) {
...
esp_err_t on9rstore::append_entry_unsafe(
    uint16_t type, const uint8_t *payload, uint32_t payload_len,
    on9rstore_def::entry_header *entry_info_out, bool force_flush)
{
    const uint64_t entry_size = get_entry_size(payload_len);
    if (entry_size == 0 ||
        entry_size >
            active_segment.data_end - active_segment.data_start) {
        return ESP_ERR_INVALID_SIZE;
    }

    if (entry_size > cfg.write_buffer_size) {
        return append_direct_entry_unsafe(
            type, payload, payload_len, entry_info_out);
    }

    return append_buffered_entry_unsafe(
        type, payload, payload_len, entry_info_out, force_flush);
}

esp_err_t on9rstore::append_entry(
    uint16_t type, const uint8_t *payload, size_t payload_len,
    on9rstore_def::entry_header *entry_info_out,
    uint32_t timeout_ticks, bool force_flush)
{
    ...
    ret = append_entry_unsafe(
        type, payload, static_cast<uint32_t>(payload_len),
        entry_info_out, force_flush);
...
esp_err_t on9rstore::flush_unsafe()
{
    ...
    esp_err_t ret = write_exact_fd(
        writer_fd, segment_file_size, write_buf_offset,
        write_buf.get(), write_buf_pos);
    if (ret == ESP_OK) {
        ret = sync_fd(writer_fd);
    }
    if (ret == ESP_OK) {
...
esp_err_t on9rstore::calculate_coredump_crc_unsafe(
    const void *partition_ptr, size_t partition_offset,
    size_t coredump_size, uint32_t *crc_out) const
{
#if CONFIG_ESP_COREDUMP_ENABLE_TO_FLASH
    ...
    while (remaining > 0) {
        ...
        esp_err_t ret =
            esp_partition_read(partition, offset, chunk, chunk_len);
        if (ret != ESP_OK) {
            return ret;
        }
...
esp_err_t on9rstore::append_coredump_entry_unsafe(
    const void *partition_ptr, size_t partition_offset,
    size_t coredump_size, uint32_t coredump_crc,
    const on9rstore_def::boot_event &base_event)
{
#if CONFIG_ESP_COREDUMP_ENABLE_TO_FLASH
    const auto *partition =
        static_cast<const esp_partition_t *>(partition_ptr);
    ...
    const uint32_t payload_len =
        sizeof(event) + static_cast<uint32_t>(coredump_size);
    const uint64_t entry_size = get_entry_size(payload_len);
    ...
    on9rstore_def::entry_header header = {};
    if (ret == ESP_OK) {
        ret = build_entry_header_unsafe(
            on9rstore_def::ENTRY_BOOT_EVENT, payload_len, &header);
    }

    const uint64_t entry_offset = active_write_offset;
    if (ret == ESP_OK) {
        ret = write_exact_fd(writer_fd, segment_file_size,
                             entry_offset, &header, sizeof(header));
    }

    uint32_t entry_crc = calc_crc32_update(
        UINT32_MAX, reinterpret_cast<const uint8_t *>(&header),
        sizeof(header));
    if (ret == ESP_OK) {
        ret = write_exact_fd(writer_fd, segment_file_size,
    ...
    if (ret == ESP_OK) {
        ret = write_entry_trailer_unsafe(
            entry_offset, payload_len, ~entry_crc, entry_size);
    }
    if (ret == ESP_OK) {
        ret = sync_fd(writer_fd);
    }
...
esp_err_t on9rstore::append_boot_entry_unsafe()
{
    ...
    if (cfg.copy_coredump &&
        ...
        if (ret == ESP_OK && partition != nullptr &&
            size <= UINT32_MAX - sizeof(event) &&
            ...
            ret = calculate_coredump_crc_unsafe(
                partition, partition_offset, size, &coredump_crc);
            if (ret == ESP_OK &&
                (state.coredump_size != size ||
                 state.coredump_crc32 != coredump_crc)) {
                ret = append_coredump_entry_unsafe(
                    partition, partition_offset, size,
                    coredump_crc, event);
                if (ret == ESP_OK) {
                    return ESP_OK;
                }
                ESP_LOGW(TAG, "Boot: coredump append failed: 0x%x",
                         ret);
            }
        }
    }
    ...
    return append_entry_unsafe(
        on9rstore_def::ENTRY_BOOT_EVENT,
        reinterpret_cast<const uint8_t *>(&event),
        sizeof(event), nullptr, true);
}
